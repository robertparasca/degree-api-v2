...
<body>
    ...
    <div>
        ...
        @endif
        @if($scholarship)
            <p>Studentul(a) <span>beneficiază</span> de bursă de <span>{{$scholarship->type}}</span> în semestrul <span>{{$isFirstSemester ? 'I' : 'II'}}</span>, an universitar <span>{{$activeYear}}</span>, cuantumul bursei fiind de <span>{{$scholarship->amount}}</span> lei/lună.</p>
        @endif
